...
class table_manage_users_waiting_period extends table_sql {
    ...
    public function __construct(string $uniqueid, int $learningplanid) {
        ...
        $columns = [
            ...
            'nextperiodname' => get_string('nextperiodname', 'local_sc_learningplans'),
    ...
    protected function init_sql() {
        $fields = 'lu.*, np.name nextperiodname, u.firstname, u.lastname, u.email,
        ...
        LEFT JOIN {local_learning_periods} np ON (np.id = lu.nextperiodid)';

        // Report search.
        $where = 'lu.learningplanid = :learningplanid AND lu.waitingperiod = 1 ';
        $params = [
    ...
    public function col_action_user($values) {
        $html = html_writer::tag('button', get_string('enrolnextperiod', 'local_sc_learningplans'), [
            'id' => 'enrolnextperiod',
            'class' => 'btn btn-primary',
            'userid' => $values->userid,
            'data-toggle' => 'tooltip',
            'data-placement' => 'bottom',
            'data-original-title' => 'Enrol User in next period',
        ]);

        $html = html_writer::link(new moodle_url('/local/sc_learningplans/usersperiod.php', ['id' => $this->learningplanid, 'userid' => $values->userid]), $html);
        return $html;
    }
}
